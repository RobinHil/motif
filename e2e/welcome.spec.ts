...
test('first launch opens "demo" and invites to the tutorial until the cross closes it for good', async () => {
  ...
  const { page } = running
  await expect(page.getByRole('list', { name: 'Tracks' }).getByRole('listitem')).toHaveCount(5)
...
test('the tutorial builds the tek track step by step, and plays it as a song', async () => {
  ...
  await expect(tutorial).toContainText('Step 1 of 11')
  ...
  for (let step = 2; step <= 11; step++) {
    await expect(tutorial).toContainText(`Step ${String(step)} of 11`)
  ...
  await expect(page.getByLabel('Song code')).toContainText(
    '$: arrange([8, intro], [8, build], [16, acid_drop], [8, melodic], [16, drop], [8, rampage], [16, industrial], [4, outro])',
  )
